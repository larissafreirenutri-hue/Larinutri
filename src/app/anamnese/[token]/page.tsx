...
import { FormularioAnamnese } from "./formulario";

export const metadata: Metadata = {
  title: "Anamnese, Larissa Freire Nutricionista",
      ...
      <main className="mx-auto my-12 max-w-[520px] px-5">
        <div className="rounded-[20px] border border-linha bg-cartao px-8 py-10 text-center shadow-cartao">
          ...
          <p className="mt-3 font-sans text-[15px] leading-relaxed text-neutro">
            Não encontramos uma anamnese aberta para este endereço. O link pode
            já ter sido respondido, ter passado do prazo, ou ter sido copiado
...
export default async function AnamnesePage({
  ...
  const { data, error } = await supabase.rpc("get_anamnese_link", {
    p_token: token,
  });
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-[640px] flex-1 px-5 py-6">
        <FormularioAnamnese token={token} primeiroNome={linha.primeiro_nome} />
      </main>
    </>
  );
}
