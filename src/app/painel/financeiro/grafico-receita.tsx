...
export function GraficoReceita({ meses }: { meses: MesResumo[] }) {
  ...
  return (
    <div className="rounded-xl border border-linha bg-cartao px-6 py-6">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h2 className="font-display text-lg text-vital-fundo">
          Receita recebida por mês
        </h2>
        <span className="font-sans text-xs text-neutro">últimos 6 meses</span>
      </div>

      {!temDados ? (
        <p className="mt-6 font-sans text-sm text-neutro">
        ...
        <ul className="mt-7 flex h-44 items-end gap-2 sm:gap-4">
          {meses.map((mes) => {
            ...
            return (
              ...
              >
                <span className="font-sans text-[10px] tabular-nums text-neutro sm:text-xs">
                  {mes.valor > 0 ? formatarMoeda(mes.valor) : ""}
                </span>
                <div
                  className="w-full rounded-t bg-vital/10 transition-all"
                ...
                />
                <span className="font-sans text-xs capitalize text-neutro">
                  {mes.rotulo}
                </span>
              </li>
            );
